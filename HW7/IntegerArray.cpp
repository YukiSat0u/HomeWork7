...
IntegerArray::IntegerArray(int length) : _length(length)
{
    if (length < 0)
    {
        throw bad_length("Error: Длина массива меньше нуля.", length);
    }
...
void IntegerArray::wash() noexcept
{
    delete[] _data;
    _data = nullptr;
    _length = 0;
...
void IntegerArray::redistribute(int newLength)
{
    if (newLength < 0)
    {
        throw bad_length("Error: Длина массива меньше нуля.", newLength);
    }
...
void IntegerArray::insertBefore(int value, int index)
{
    ...
    {
        throw bad_range("Error: Индекс вне допустимого диапазона", _length, index);
    }
